...
本仓库是全线 Apple App 的瞬态反馈（toast）正身。范围裁决与不变式记录在此；README 是使用文档。同目录 `CLAUDE.md` 是指向本文件的兼容符号链接。

## 不变式（改动前先读）

1. **零配置是产品决策，不是省事。** 视觉（capsule、iOS 26+ 原生非交互 Liquid Glass、
   旧版 iOS 与 macOS 使用系统语义背景、默认无 icon）、位置
   ...
   逐调用点自由度只有两个：`duration`（有真实需求：CodeCat 的登录警告需要 10s 阅读时间）
   和 `showsIcon`（默认 `false`；个别 toast 需要语义标时再打开）。
